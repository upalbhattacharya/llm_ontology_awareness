    ...
    for ont in "${ONTOLOGIES[@]}"; do
        for r_args in "${BASE_RESULTS_PATH}/${shot}_shot${STRAT_SUFFIX}/${MODEL}/${ont}"/*; do
            for run in "${r_args}/runs"/*; do
                # echo ${run}
                python3 ${HOME}/PhD/Experiments/llm_ontology_awareness/src/llm_ontology_awareness/model/open_ai/format_response_content/term_typing.py -f ${run}/batch_output_1.jsonl -r ${r_args}/params.json -l ${r_args}/label_mapping.json
            done
        done
    done
done
